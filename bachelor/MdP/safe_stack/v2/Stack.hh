...
Stack<T>::swap(Stack& y) noexcept {
  //Funziona correttamente per tutti i tipi built-in
  using std::swap;
  swap(vec_, y.vec_);
  swap(size_, y.size_);
...
template<typename T>
inline 
Stack<T>& Stack<T>::operator=(Stack&& y) noexcept {
  assert(check_inv() && y.check_inv());
  Stack temp(std::move(y));
  //Siccome il valore di this viene spostato dentro alla variabile temporanea
  //temp andando out of scope alla fine dell'operatore viene automaticamente distrutto
  //e le risorse liberate
  swap(temp);
  assert(check_inv());
  return *this;
}
...
//il secondo typename assicure il compilatore che value_type è un alias di un typename
// e che quindi è risolvibile a compile-time, invece che un valore
template<typename T>
inline 
...
//// IMPLEMENTATION (was in cc file before)
...
Stack<T>::make_copy(const value_type* src,
  ...
  try {
	//Qui dst è sotto la nostra responsabilità
