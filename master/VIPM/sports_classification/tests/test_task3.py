...
class TestLoadDataset:
    def test_returns_three_elements(self, tmp_path):
        ...
        paths, labels, class_names = oneshot.load_dataset(str(tmp_path))
        assert len(paths) == 6
        assert len(labels) == 6
    ...
    def test_labels_are_contiguous_ints(self, tmp_path):
        ...
        _, labels, _ = oneshot.load_dataset(str(tmp_path))
    ...
    def test_class_names_are_sorted(self, tmp_path):
        for cls in ['zzz', 'aaa', 'mmm']:
            ...
            import cv2
            cv2.imwrite(str(d / "0.jpg"), img)
        _, _, class_names = oneshot.load_dataset(str(tmp_path))
...
class TestExtractDenseSift:
    def test_returns_2d_float32_array(self):
        img = make_gray()
        descs = oneshot.extract_dense_sift(img, step=8)
        assert descs.ndim == 2
    ...
    def test_larger_step_gives_fewer_descriptors(self):
        img = make_gray(224, 224)
        d8 = oneshot.extract_dense_sift(img, step=8)
        d16 = oneshot.extract_dense_sift(img, step=16)
...
class TestBuildCodebook:
    def test_cluster_centers_shape(self):
        ...
        kmeans = oneshot.build_codebook(descs, K=50)
...
class TestEncodeSpm:
    def setup_method(self):
        self.K = 50
        self.kmeans = oneshot.build_codebook(make_descs(5000), K=self.K)

    def test_output_shape_is_k_times_21(self):
        feat = oneshot.encode_spm(make_gray(), self.kmeans, K=self.K)
    ...
    def test_different_images_differ(self):
        f1 = oneshot.encode_spm(make_gray(seed=0), self.kmeans, K=self.K)
        f2 = oneshot.encode_spm(make_gray(seed=99), self.kmeans, K=self.K)
...
class TestTrainClassifier:
    ...
    def test_returns_clf_and_five_cv_scores(self):
        X, y = self._make_data()
        clf, scores = oneshot.train_classifier(X, y, C=1.0)
        assert scores.shape == (5,)
        assert hasattr(clf, 'predict')

    def test_clf_is_fitted_on_full_data(self):
        X, y = self._make_data()
        clf, _ = oneshot.train_classifier(X, y, C=1.0)
...
class TestEvaluate:
    ...
    def test_returns_accuracy_and_f1(self):
        ...
        m = oneshot.evaluate(clf, X, y, [str(i) for i in range(10)])
...
class TestSaveConfusionMatrix:
    def test_creates_png_file(self, tmp_path):
        ...
        oneshot.save_confusion_matrix(cm, ['a', 'b', 'c'], out)
...
class TestLogResults:
    def test_appends_to_log(self, tmp_path, monkeypatch):
        ...
        oneshot.log_results('test-tag', clean, degraded, scores)
