...
// compilazione: g++ arith_dot.cc -o arith_dot
//
// Obiettivo:

// 1) eval bottom up di espressione aritmetica in notazione polacca inversa (./a.out 1 2 3 4 + + +)
// E ::= N | E E OP   (Notazione polacca inversa -> Postfissa)
// E ::= N | (E OP E)  (Notazione infissa)
//
// L'albero di derivazione è lo stesso,
// - visitandolo in ordine da sinistra a destra si ottiene una notazione infissa
// - visitandolo in post-ordine si ottiene una notazione postfissa
...
void print_status(list_t *l, node_t *n, string c)
{
	...
	if (n == NULL)
	{ /// di solito se chiedo solo la lista e' l'ultima operazione della sequenza -> coloro di verde lo sfondo
...
int eval_top_down(my_stack *s)
{
	...
	{ /// somma
		...
		val1 = eval_top_down(s);
		output_graph << "rec_node_" << addr_head << ":e -> rec_node_" << get_address(s->head) << " [label=" << ct_rec++ << "];\n";
		val2 = eval_top_down(s);
		return val1 + val2;
	}
	if (op == 2)
	{ /// -
...
int main(int argc, char **argv)
{
	...
	if (graph)
	{
		output_graph.open("graph_bottom_up.dot");
		/// preparo header
	...
	if (graph)
	{
		...
		output_graph << "}" << endl;
		output_graph.close();
		cout << " File graph_bottom_up.dot scritto" << endl;
	}
	...
	if (graph)
	{
		output_graph.open("graph_top_down.dot");
		/// preparo header
		output_graph << "digraph g" << endl;
		output_graph << "{ " << endl;
		output_graph << "node [shape=none]" << endl;
	...
	idx = 0;
	arg_ct = 1; // scorre tutti gli argomenti
