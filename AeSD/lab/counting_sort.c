...
// compilazione: gcc .\counting_sort.c -o counting
//
// Obiettivo:
...
int main(int argc, char **argv)
{
  ...
  /// allocazione array
  A = malloc(max_dim * sizeof(int));
  B = malloc(max_dim * sizeof(int));
  C = malloc(MAX_COUNT * sizeof(int)); /// alloco l'array per i conteggi
  ...
  free(A);
  free(B);
  free(C); /// dealloco l'array di supporto per i conteggi (dipende da k = massimo nell'array in uso)
